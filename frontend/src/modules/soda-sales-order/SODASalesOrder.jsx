...
import { sodaSalesOrderCopyFromApi, normaliseDocumentHeader, normaliseDocumentLine, unwrapCopyFromDocument, BASE_TYPE } from '../../api/copyFromApi';
import {
...
function SODASalesOrder() {
            ...
            try {
                ...
                if (!ignore) {
                    ...
                    setFormSettings((prev) => ({
                        ...
                        matrixColumns: buildCompanyMatrixColumnSettings(rawRowUdfs, prev.matrixColumns),
                        rowUdfs: {
    ...
    useEffect(() => {
        const docEntry =
            location.state?.sodaSalesOrderDocEntry ||
        ...
        const load = async () => {
            ...
            try {
                ...
                if (!ignore) setPageState(p => ({ ...p, error: getErrMsg(e, 'Failed to load SODA sales order.') }));
            } finally {
                if (!ignore) {
    ...
    const openItemModalSafe = async (lineIndex) => {
        ...
        try {
            ...
            console.error('Failed to load items for SODA Sales Order modal:', error);
            setItemModal((prev) => ({
                ...prev,
    ...
    useEffect(() => {
        ...
        const persistedCopyState = routedCopyFrom ? null : consumeCopyToState(location.pathname, ['/soda-sales-order']);
    ...
    const fetchCopyFromDocuments = async (docType) => {
        try {
            const buyerCode = String(header.vendor || '').trim();
            if (!buyerCode) return [];
            return await sodaSalesOrderCopyFromApi.fetchOpenDocuments(docType, buyerCode);
        } catch (error) {
    ...
    const fetchCopyFromDocumentDetails = async (docType, docEntry) => {
        try {
            return await sodaSalesOrderCopyFromApi.fetchDocumentForCopy(docType, docEntry);
        } catch (error) {
    ...
    const handleCopyTo = async (targetType) => {
        await copyToDocument({
            ...
            errorMessage: pageState.loading
                ? 'Please wait until the SODA sales order has finished loading before using Copy To.'
                : 'Open a saved SODA sales order before using Copy To.',
        });
    };
    ...
    const handleDuplicate = () => {
        const duplicated = duplicateDocumentInPlace({
            ...
            navigate,
            location,
            successMessage: 'SODA sales order duplicated. Review and add it as a new entry.',
        });
    ...
    const openFindPage = () => {
        navigate('/soda-sales-order/find', { state: null });
    };

    const visHdrUdfs = headerUdfDefinitions.filter(f => formSettings.headerUdfs?.[f.key]?.visible !== false);
    const visibleRowUdfs = rowUdfDefinitions.filter(f => formSettings.rowUdfs?.[f.key]?.visible === true);
