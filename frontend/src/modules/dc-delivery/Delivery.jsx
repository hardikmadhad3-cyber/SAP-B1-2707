...
import { fetchSalesOrderByDocEntry, fetchSalesOrderVendorDetails } from '../../api/dcSalesOrderApi';
import { fetchHSNCodeFromItem } from '../../api/hsnCodeApi';
import { SALES_ORDER_COMPANY_ID } from '../../config/appConfig';
import { dcDeliveryCopyFromApi, normaliseDocumentHeader, normaliseDocumentLine, BASE_TYPE } from '../../api/copyFromApi';
import {
...
function DCDelivery() {
  ...
  useEffect(() => {
    ...
    const load = async () => {
      ...
      try {
        ...
        setPageState(p => ({ ...p, success: so.doc_num ? `DC Delivery ${so.doc_num} loaded.` : 'DC Delivery loaded.' }));
      } catch (e) {
        if (!ignore) setPageState(p => ({ ...p, error: getErrMsg(e, 'Failed to load DC delivery.') }));
      } finally {
  ...
  useEffect(() => {
    ...
    const persistedCopyState = consumePersistedCopyToState(location.pathname, ['/dc-delivery/new', '/dc-delivery']);
    ...
    // Populate header for a new DC Delivery copied from the source document.
    setHeader(prev => ({
      ...INIT_HEADER,
      postingDate: today(),
      documentDate: today(),
  ...
  const validate = async () => {
    ...
    try {
      ...
      if (backendErrors.length > 0) {
        ...
        e.form = getErrMsg(validationError, 'DC Delivery validation failed.');
      }
    }
  ...
  const handleCopyFrom = (data, sourceType) => {
    ...
    const labels = { salesOrder: 'DC Sales Order', dcSalesOrder: 'DC Sales Order', salesQuotation: 'Sales Quotation', delivery: 'Delivery', dcDelivery: 'DC Delivery', returns: 'Return', blanket: 'Blanket Agreement' };
  ...
  const fetchCopyFromDocuments = async (docType) => {
    try {
      ...
      return await dcDeliveryCopyFromApi.fetchOpenDocuments(docType, bpCode);
    } catch (err) {
  ...
  const fetchCopyFromDocumentDetails = async (docType, docEntry) => {
    try {
      ...
      return await dcDeliveryCopyFromApi.fetchDocumentForCopy(docType, docEntry);
    } catch (err) {
  ...
  const handleCopyTo = async (targetType) => {
    await copyToDocument({
      ...
      restoreState: { dcDeliveryDocEntry: currentDocEntry },
      navigate,
      upsertTask,
      removeTask,
      ...
      errorMessage: 'Please save the DC delivery first before copying to another document.',
    });
  };
  ...
  const handleDuplicate = () => {
    ...
    const duplicated = duplicateDocumentInPlace({
      ...
      navigate,
      location,
      successMessage: 'DC delivery duplicated. Review and add it as a new entry.',
    });
  ...
  const handleSubmit = async (ev) => {
    ...
    try {
      ...
      const payload = {
        ...
        lines: lines.map((line) => {
          ...
          return {
            ...
            udf: buildVisibleEnteredRowUdfPayload(rowUdfDefinitions, line.udf || {}, dcDeliveryFormSettings),
          };
        }),
      ...
      const message = getErrMsg(e, 'DC Delivery submission failed.');
  ...
  const dcDeliveryFormSettings = useMemo(() => {
  ...
  const visibleRowUdfs = filteredRowUdfDefinitions.filter(f => dcDeliveryFormSettings.rowUdfs?.[f.key]?.visible === true);
  ...
  useDocumentSeries({ endpoint: '/dc-delivery', companyKey: refData.company, currentDocEntry, header: header, setHeader, setRefData, setPageState, ready: !pageState.loading && !pageState.posting , refreshKey: seriesRevision});
  ...
  return (
    <form ref={formRef} className={`del-page sap-document-page${isRightSidebarOpen ? ' del-page--sidebar-open' : ''}`} onSubmit={handleSubmit} onChangeCapture={markDirty}>
      ...
      <div className="del-toolbar sap-document-toolbar">
        ...
        <div className="del-dropdown" style={{ position: 'relative', display: 'inline-block' }}>
          ...
          <div className="del-dropdown-menu">
            {[
              ...
              { key: 'salesOrder',     label: 'DC Sales Orders' },
        ...
        <button type="button" className="del-btn sap-document-toolbar__find" onClick={() => navigate('/dc-delivery/find')}>Find</button>
