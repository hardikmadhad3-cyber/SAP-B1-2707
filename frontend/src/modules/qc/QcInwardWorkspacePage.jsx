...
export default function QcInwardWorkspacePage() {
  ...
  const loadVendors = async () => {
    setLoading(true);
    try {
      const data = await listInwardPendingVendors();
      ...
      if (!nextVendors.length && meta?.error) {
        showAlert('error', `Vendor load debug: ${meta.error}`);
      }
    } catch (error) {
      showAlert('error', error?.response?.data?.message || 'Failed to load pending vendors.');
      setVendors([]);
      setVendorMeta(null);
    } finally {
      setLoading(false);
  ...
  useEffect(() => {
    const loadDocuments = async () => {
      ...
      setLoading(true);
      try {
        const data = await listInwardPendingDocuments(selectedVendor);
        ...
        showAlert('error', error?.response?.data?.message || 'Failed to load pending GRPO documents.');
        setDocuments([]);
      } finally {
        setLoading(false);
      }
  ...
  useEffect(() => {
    const loadItems = async () => {
      ...
      setLoading(true);
      try {
        const data = await listInwardPendingItems(selectedDocEntry);
        const rows = Array.isArray(data?.items) ? data.items : [];
        setPendingItems(rows);
      } catch (error) {
        showAlert('error', error?.response?.data?.message || 'Failed to load pending items for GRPO.');
        setPendingItems([]);
      } finally {
        setLoading(false);
      }
  ...
  const loadDetailForItem = async (row) => {
    ...
    try {
      const [parametersData, batchData] = await Promise.all([
        listInwardItemParameters(row.item_code),
        row.batch_managed ? listInwardItemBatches(row.item_code, row.whs_code) : Promise.resolve({ batches: [] }),
      ]);
  ...
  const selectedVendorName = useMemo(
    () => vendors.find((row) => row.vendor_code === selectedVendor)?.vendor_name || '',
  ...
  const refreshCurrentData = async () => {
    ...
    const docsData = await listInwardPendingDocuments(selectedVendor);
    ...
    const itemsData = await listInwardPendingItems(selectedDocEntry);
  ...
  const saveQc = async () => {
    if (!selectedItem) {
      showAlert('error', 'Select a pending GRPO item first.');
      return;
    }
    ...
    try {
      ...
      showAlert('success', 'Inward QC saved successfully.');
      await refreshCurrentData();
    } catch (error) {
      showAlert('error', error?.response?.data?.message || 'Failed to save inward QC.');
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="qc-page sap-document-page qc-workspace qc-inward-page">
      <div className="qc-toolbar">
        <div className="qc-workspace__toolbar-head">
          <span className="qc-toolbar__title">Inward QC</span>
          <span className="qc-workspace__toolbar-subtitle">Pending GRPO -> Item Inspection -> Save QC</span>
        </div>
        <button type="button" className="qc-btn" onClick={loadVendors} disabled={loading}>Refresh</button>
      </div>
      ...
      <div className="qc-grid qc-workspace__block qc-inward-page__pending-card">
        ...
        <div className="qc-workspace__transaction-form qc-inward-page__filters">
          <label>
            <span>Vendor (Pending GRPO only)</span>
            <select
              className="qc-select"
            ...
            {!vendors.length && vendorMeta ? (
              <small className="qc-workspace__meta-note">
                Debug: DB {String(vendorMeta.resolved_database || 'default')} | Open GRPO Docs {String(vendorMeta.open_grpo_documents || 0)}
              </small>
            ) : null}
          </label>

          <label>
            <span>Pending GRPO Document</span>
            <select
              className="qc-select"
          ...
          <label>
            ...
            <input className="qc-input" value={selectedVendorName} readOnly />
          </label>
          ...
          <table>
            ...
            <tbody>
              {pendingItems.length ? pendingItems.map((row, index) => {
                ...
                <tr>
                  <td colSpan={10} className="qc-empty">No pending GRPO QC items found.</td>
                </tr>
              )}
            </tbody>
          </table>
