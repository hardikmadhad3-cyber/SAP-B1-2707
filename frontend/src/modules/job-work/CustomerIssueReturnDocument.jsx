...
const TRANSACTION = JOB_WORK_TRANSACTIONS.customerIssueReturnNote;
...
// Backend stores the linked Inventory Goods Issue reference in a dedicated
// field pair (see jobWorkSchema.js's WebGoodsIssDocEntry/DocNum) — separate
// from the legacy GoodsIssueN/GISeries free-text fields, which aren't a real SAP link.
...
export default function CustomerIssueReturnDocument() {
  ...
  // object STTL_JWSR), one for the linked Inventory Goods Issue it creates
  // (SAP object 60) — two separate SAP documents, two separate series lists.
  ...
  useEffect(() => {
    if (docEntry || !goodsIssueSeriesOptions.length) return;
    setHeader((current) => {
      if (current.GoodsIssueSeries) return current;
      const defaultSeries = goodsIssueSeriesOptions.find((s) => s.isDefault) || goodsIssueSeriesOptions[0];
      return defaultSeries ? { ...current, GoodsIssueSeries: defaultSeries.Series } : current;
    });
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [goodsIssueSeriesOptions, docEntry]);
  ...
  // Reciprocal golden arrow entry point (matches the Customer Receipt Note
  // pattern) — reserved for whenever another page links back here with
  // { customerIssueDocEntry } in route state.
  useEffect(() => {
    const targetDocEntry = location.state?.customerIssueDocEntry;
  ...
  // Batch handling mirrors the real Goods Issue page (frontend/src/modules/goods-issue/GoodsIssue.jsx):
  // BatchAllocationModal in "issue" mode (its default) lets the user pick
  // FROM EXISTING batches already on hand at that warehouse, rather than
  // creating new ones — this document draws down existing inventory, it
  // doesn't add any.
  const openBatchModal = async (lineIndex) => {
    ...
    try {
      const response = await fetchGoodsIssueBatchesByItem(line.ICode, line.WhsCode);
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const goodsIssueDocNum = data.stock_movements?.goodsIssue?.docNum;
      const successMessage = `${data.message || 'Customer Issue / Return Note saved.'}`
        + `${data.doc_num ? ` Doc No: ${data.doc_num}.` : ''}`
        + `${goodsIssueDocNum ? ` Goods Issue Doc No: ${goodsIssueDocNum}.` : ''}`;
      if (data.warning) {
        // eslint-disable-next-line no-alert
        window.alert(data.warning);
      }
      // Reopen the saved document (rather than resetting to the list) so any
      // linked Inventory Goods Issue created by the save is immediately visible.
    ...
    return (
      <div className="container-fluid sap-find-page">
        <div className="d-flex justify-content-between align-items-center mb-4">
          <div>
            <h2 className="mb-1">Customer Issue / Return Note</h2>
            <small className="text-muted">Material issued back out to — or returned by — a customer after job work.</small>
          </div>
        ...
        <div className="card p-3 sap-find-card">
          ...
          <div className="table-responsive sap-find-table-wrap">
            <table className="table table-bordered table-hover align-middle mb-0">
              ...
              <tbody>
                {listLoading && (
                  <tr><td colSpan={6} className="text-center py-4">Loading Customer Issue / Return Notes...</td></tr>
                )}
                {!listLoading && !documents.length && (
                  <tr><td colSpan={6} className="text-center text-muted py-4">No Customer Issue / Return Notes found.</td></tr>
                )}
  ...
  const branchFilteredWarehouses = header.JWBranch
    ...
    : referenceData.warehouses;
  const { goodsIssueDocEntry, goodsIssueDocNum } = getStockMovementRefs(header);
  // Once the real Inventory Goods Issue has been posted, everything that
  // could desync from what SAP actually moved (customer, dates, branch,
  // warehouse, bins, lines) is locked — only the informational fields in
  // LOCKED_EDITABLE_FIELDS stay editable, and Update never re-posts.
  const isLocked = Boolean(docEntry) && Boolean(goodsIssueDocEntry);
  const openLinkedGoodsIssue = () => {
    if (!goodsIssueDocEntry) return;
    navigate('/goods-issue', {
      state: createActiveCompanyScopedRouteState({ goodsIssueDocEntry }),
    });
  };
    ...
    <div className="po-page jw-receipt-note-page">
      <div className="po-toolbar">
        <div className="po-toolbar__title">
          Customer Issue / Return Note{docEntry ? ` - #${docEntry}` : ''}
        </div>
        ...
        {goodsIssueDocEntry ? (
          <PrintLayoutToolbar
            documentType="inventoryGenExit"
            documentLabel="Inventory Goods Issue"
            docEntry={goodsIssueDocEntry}
            docNumber={goodsIssueDocNum}
            classPrefix="po"
      ...
      <div className="po-header-card jw-header-card">
        {isLocked ? (
          <div className="jw-header-lockband">
            🔒 Stock already issued for this document — customer, dates, warehouse and bins are locked. Only the logistics fields below can still be edited.
          </div>
        ) : null}
        ...
        <div className="jw-header-grid">
          <div className="jw-header-column">
            ...
            <div className="po-field jw-field">
              ...
              <select className="po-field__select" value={header.PTerm || ''} disabled={isLocked} onChange={(e) => handleHeaderChange('PTerm', e.target.value)}>
                <option value="">Select Payment Term</option>
                {referenceData.paymentTerms.map((term) => (
                  <option key={term.GroupNum} value={term.GroupNum}>{term.PymntGroup}</option>
                ))}
              </select>
            </div>
            <div className="po-field jw-field">
              <label className="po-field__label">Sales Employee</label>
              <select className="po-field__select" value={header.SEmp || ''} disabled={isLocked} onChange={(e) => handleHeaderChange('SEmp', e.target.value)}>
            ...
            <div className="po-field jw-field">
              <label className="po-field__label">Customer PO No</label>
              <input className="po-field__input" value={header.CPONo || ''} disabled={isLocked} onChange={(e) => handleHeaderChange('CPONo', e.target.value)} />
            </div>
            <div className="po-field jw-field">
              <label className="po-field__label">Customer PO Date</label>
              <input type="date" className="po-field__input" value={header.CPODT || ''} disabled={isLocked} onChange={(e) => handleHeaderChange('CPODT', e.target.value)} />
            </div>
            <div className="po-field jw-field">
              <label className="po-field__label">Return Type</label>
              <input className="po-field__input" value={header.JWRtn || ''} disabled={isLocked} onChange={(e) => handleHeaderChange('JWRtn', e.target.value)} />
            </div>
            <div className="po-field jw-field">
            ...
            <div className="po-field jw-field">
              ...
              {goodsIssueDocEntry ? (
                <div className="jw-field__selector">
                  ...
                  <button
                    type="button"
                    onClick={openLinkedGoodsIssue}
                    title="Open Goods Issue"
                    style={{
                ...
                <div className="jw-field__selector">
                  ...
                  <input
                    className="po-field__input"
                    style={{ maxWidth: 90 }}
                    value={header.GoodsIssueSeries ? (goodsIssueSeriesOptions.find((s) => String(s.Series) === String(header.GoodsIssueSeries))?.NextNumber ?? '—') : ''}
                    placeholder="Next No"
                    readOnly
                  />
                </div>
      ...
      <div className="po-tab-panel">
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 10 }}>
          ...
          {isLocked ? (
            <span className="po-mode-badge" title="The Inventory Goods Issue for this document has already been posted — items, quantities, warehouse and bins can no longer change.">Locked — stock movement posted</span>
          ) : (
                  ...
                  <td>
                    <div style={{ display: 'flex', gap: 2, alignItems: 'center' }}>
                      <input
                        ...
                        readOnly
                        placeholder={line._batchManaged ? 'Select batch' : 'Batch'}
                      />
                      <button
                        type="button"
