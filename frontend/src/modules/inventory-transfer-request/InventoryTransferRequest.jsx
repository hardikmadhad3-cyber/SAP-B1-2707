...
function InventoryTransferRequest() {
    ...
    const load = async () => {
      ...
      try {
        const [
          ...
          fetchInventoryTransferRequestItems(),
          fetchInventoryTransferRequestWarehouses(),
          fetchInventoryTransferRequestSeries({ date: today() }),
        ]);
  ...
  useEffect(() => {
    let ignore = false;
    fetchInventoryTransferRequestSeries({ date: header.postingDate, branch: header.fromBranch })
      .then((response) => {
        if (ignore) return;
  ...
  const handleDuplicate = () => {
    const duplicated = duplicateDocumentInPlace({
      ...
      navigate,
      location,
      successMessage: 'Inventory Transfer Request duplicated. Review and add it as a new entry.',
    });
