...
const SCHEMA_PATH = path.join(BACKEND_ROOT, 'db', 'whatsapp-schema.sqlite.sql');
...
const resolveCompanyId = () => {
  ...
  if (!Number.isInteger(companyId) || companyId <= 0) {
    throw new Error('A valid company session is required for WhatsApp data.');
  }
  return companyId;
};

const resolveSqlitePath = (companyId = resolveCompanyId()) => {
  const configuredPath = process.env.WHATSAPP_SQLITE_PATH || './data/whatsapp.sqlite';
...
const ensureSchema = () => {
  ...
  console.log(`[WHATSAPP_DB] SQLite connected to ${dbPath}`);
};
