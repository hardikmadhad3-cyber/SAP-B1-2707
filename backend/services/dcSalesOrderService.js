...
const DC_SALES_ORDER_LINE_UDFS = [
  {
    tableName: 'RDR1',
    name: 'S_BrokPerQty',
...
const getExistingLineUdfs = async () => {
  const rows = await db.query(`
    ...
    FROM CUFD
    WHERE TableID = 'RDR1'
      AND AliasID IN (${DC_SALES_ORDER_LINE_UDFS.map((_, index) => `@alias${index}`).join(', ')})
  `, DC_SALES_ORDER_LINE_UDFS.reduce((params, udf, index) => {
...
const ensureDCSalesOrderLineUdfs = async ({ warnOnly = false } = {}) => {
  if (!ensureUdfsPromise) {
    ensureUdfsPromise = (async () => {
      const existingAliases = await getExistingLineUdfs();
      const missing = DC_SALES_ORDER_LINE_UDFS.filter(
      ...
      for (const udf of missing) {
        try {
          ...
          console.log(`[DC Sales Order] Created missing row UDF ${udf.fieldName}.`);
        } catch (error) {
  ...
  try {
    ...
    console.warn('[DC Sales Order] Unable to create missing row UDFs:', error.message || error);
  }
};

const getReferenceData = async (...args) => {
  await ensureDCSalesOrderLineUdfs({ warnOnly: true });
  return salesOrderService.getReferenceData(...args);
};

const submitSalesOrder = async (...args) => {
  await ensureDCSalesOrderLineUdfs();
  return salesOrderService.submitSalesOrder(...args);
};

const updateSalesOrder = async (...args) => {
  if (args[1]?.confirmation_only !== true) await ensureDCSalesOrderLineUdfs();
