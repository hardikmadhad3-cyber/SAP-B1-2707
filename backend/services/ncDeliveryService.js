...
const getExistingLineUdfs = async () => {
  const rows = await db.query(`
    ...
    FROM CUFD
    WHERE TableID = 'DLN1'
      AND AliasID IN (${NC_DELIVERY_LINE_UDFS.map((_, index) => `@alias${index}`).join(', ')})
  `, NC_DELIVERY_LINE_UDFS.reduce((params, udf, index) => {
...
const ensureNCDeliveryLineUdfs = async ({ warnOnly = false } = {}) => {
  if (!ensureUdfsPromise) {
    ensureUdfsPromise = (async () => {
      const existingAliases = await getExistingLineUdfs();
      const missing = NC_DELIVERY_LINE_UDFS.filter(
      ...
      for (const udf of missing) {
        try {
          ...
          console.log(`[NC Delivery] Created missing row UDF ${udf.fieldName}.`);
        } catch (error) {
  ...
  try {
    ...
    console.warn('[NC Delivery] Unable to create missing row UDFs:', error.message || error);
  }
};

const getReferenceData = async (...args) => {
  await ensureNCDeliveryLineUdfs({ warnOnly: true });
