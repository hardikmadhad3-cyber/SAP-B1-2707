...
const resolveDocumentSeries = async (header, series, isManualSeries) => {
  ...
  const result = await serviceApCreditMemoDb.getDocumentSeries({
    date: postingDate,
...
const validatePayload = async (payload, docEntry = null) => {
  ...
  const vendor = await serviceApCreditMemoDb.getVendorValidation(vendorCode);
  if (!vendor || vendor.CardType !== 'S' || String(vendor.FrozenFor || '').toUpperCase() === 'Y') {
    throw new Error('Invalid vendor');
  }

  const postingPeriod = await serviceApCreditMemoDb.getPostingPeriodValidation(postingDate);
  if (!postingPeriod) throw new Error('Posting Date must be within open posting period');

  const branchesEnabled = await serviceApCreditMemoDb.getBranchEnabled();
  if (branchesEnabled && !String(header.branch || '').trim()) {
    throw new Error('Branch is required');
  }

  if (header.salesContractNo && await serviceApCreditMemoDb.isDuplicateVendorInvoiceNumber(vendorCode, String(header.salesContractNo).trim(), docEntry)) {
  ...
  for (const line of populatedLines) {
    ...
    const taxCodeRow = await serviceApCreditMemoDb.getTaxCodeValidation(taxCode);
...
const buildSapPayload = async (payload, includeSeries = true, docEntry = null) => {
  ...
  if ((!taxInvoiceNo || !taxInvoiceDate) && requiresTaxInvoiceReference(gstTransactionType)) {
    const baseInvoiceEntries = [...new Set(lines
      .filter((line) => optNumber(line.baseType) === 18 && optNumber(line.baseEntry) !== undefined)
      .map((line) => optNumber(line.baseEntry)))];

    if (baseInvoiceEntries.length === 1) {
      const baseReference = await serviceApCreditMemoDb.getAPInvoiceTaxReference(baseInvoiceEntries[0]);
  ...
  if (requiresTaxInvoiceReference(gstTransactionType) && (!taxInvoiceNo || !taxInvoiceDate)) {
    throw new Error('Original Invoice No. and Original Invoice Date are required for a GST credit memo. Enter them on the Tax tab or use Copy From A/P Invoice.');
  }
...
const getReferenceData = async () => loadDocumentCurrencyReferenceData(
  {},
  await serviceApCreditMemoDb.getReferenceData(),
);

module.exports = {
  getReferenceData,
  getVendorDetails: serviceApCreditMemoDb.getVendorDetails,
  getVendorFilterOptions: apInvoiceService.getVendorFilterOptions,
  getDocumentSeries: serviceApCreditMemoDb.getDocumentSeries,
  getNextNumber: serviceApCreditMemoDb.getNextNumber,
  getServiceAPCreditMemoList: serviceApCreditMemoDb.getServiceAPCreditMemoList,
  getServiceAPCreditMemo: async (...args) => normalizeCopyDocumentRate(
    await serviceApCreditMemoDb.getServiceAPCreditMemo(...args),
    await loadDocumentCurrencyReferenceData(),
  ),
  submitServiceAPCreditMemo,
  updateServiceAPCreditMemo,
  getOpenServiceAPInvoices: async (vendorCode) => ({ documents: await serviceApCreditMemoDb.getOpenServiceAPInvoices(vendorCode) }),
  getServiceAPInvoiceForCopy: async (...args) => normalizeCopyDocumentRate(
    await serviceApCreditMemoDb.getServiceAPInvoiceForCopy(...args),
    await loadDocumentCurrencyReferenceData(),
  ),
};
