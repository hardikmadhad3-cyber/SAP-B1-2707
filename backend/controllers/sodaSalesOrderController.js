...
const salesOrderService = require('../services/sodaSalesOrderService');
...
const getReferenceData = async (req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to load SODA sales order reference data.'));
  }
};
...
const getSalesOrderList = async (req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to load SODA sales orders.'));
  }
};
...
const getSalesOrderFilterOptions = async (req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to load SODA sales order filter options.'));
  }
};
...
const getSalesOrder = async (req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to load SODA sales order details.'));
  }
};
...
const submitSalesOrder = async (req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to submit SODA sales order.'));
  }
};
...
const updateSalesOrder = async (req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to update SODA sales order.'));
  }
};

const getDocumentSeries = async (req, res) => {
  try {
    const data = await resolveMarketingDocumentSeries({ db: seriesDatabase, objectCode: '17', targetDate: req.query.date, branch: req.query.branch || '', docSubType: req.query.docSubType, transactionType: req.query.transactionType });
    res.json(data);
  } catch (error) {
...
const getNextNumber = async (req, res) => {
  try {
    res.json(await getDocumentSeriesNumberPreview({ db: seriesDatabase, objectCode: '17', seriesId: req.query.series ?? req.params.series,
...
const getSalesOrderPrintLayouts = async (_req, res) => {
  try {
    ...
    res.status(500).json(getErrorPayload(error, 'Failed to load SODA sales order print layouts.'));
  }
};
...
const printSODASalesOrder = async (req, res) => {
  try {
    ...
    res.status(statusCode).json(getErrorPayload(error, 'Failed to generate SODA sales order PDF.'));
  }
};
